    ...
    <Section id="contact" bordered>
      <Reveal>
        <SectionHeading
          index="06"
          title="Get in touch"
          description={`I am open to Game, Mobile and Fullstack engineering work. ${profile.yearsOfExperience} years of shipped projects, and I am happy to walk through any of them in detail.`}
        />
      </Reveal>

      <ul className="grid gap-3 sm:grid-cols-2">
        {contactLinks.map((link, index) => {
          const iconName = ICON_BY_LABEL[link.label] ?? "external";
          ...
          return (
            <Reveal as="li" key={link.label} delay={index * 50}>
              {isLink ? (
      ...
      {hasPlaceholders ? (
        <Reveal>
          <PlaceholderNote>
            The source CV contains no contact details, so these are placeholders. Fill in{" "}
            <code className="font-mono text-amber-300/90">contactLinks</code> in{" "}
            <code className="font-mono text-amber-300/90">src/content/profile.ts</code> — add a{" "}
            <code className="font-mono text-amber-300/90">href</code> and remove{" "}
            <code className="font-mono text-amber-300/90">isPlaceholder</code> to make each one a
            live link.
          </PlaceholderNote>
        </Reveal>
      ) : null}
